...
type NotifyKitAndroidConfig = {
  ...
  showTimestamp?: boolean; // show timestamp when timestamp is set
  timestamp?: number; // milliseconds; used for display and notification-panel sorting
};
...
Android notification fields, including `showTimestamp` and `timestamp`, are serialized inside the `_v: 1` `notifee_options` blob and reconstructed client-side by `handleFcmMessage`. They are not mapped to Firebase Admin's `android.notification` object.
